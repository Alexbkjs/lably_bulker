...
## What this is
Chrome extension (Manifest V3) with a side panel UI for bulk managing labels and badges in the Lably Shopify app (lably.devit.software). Supports export, import, and bulk update operations.
...
3. **background.js** - Service worker that persists session data to `chrome.storage.local` and opens side panel on action click
...
- **Lably API calls**: `chrome.scripting.executeScript` inside the Lably iframe (found via `chrome.webNavigation.getAllFrames`). This is required because the Lably server rejects requests not from its own origin. `GM.xmlHttpRequest` (Tampermonkey) can set Origin header but Chrome extension fetch cannot.
...
## File overview
- `manifest.json` - Extension config, permissions: sidePanel, storage, activeTab, scripting, tabs, webNavigation
- `inject.js` - Fetch hook in admin.shopify.com MAIN world
- `content.js` - Message bridge (ISOLATED world to chrome.runtime)
- `background.js` - Service worker, session data persistence
- `sidepanel.html` - UI structure
- `sidepanel.css` - Styling with light/dark theme via CSS variables
- `sidepanel.js` - All sidepanel logic (token, API calls, UI rendering, export/import/update)
